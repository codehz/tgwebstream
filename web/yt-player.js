...
this.YouTubePlayer = (() => {
   ...
   */
  return class YouTubePlayer extends EventEmitter {
    #id = null;
    #opts = null;
    #api = null;
    #autoplay = false; // autoplay the first video?
    #player = null;
    #ready = false; // is player ready?
    #queue = [];
    #interval = null;
    #start = 0;
    ...
    constructor(element, opts) {
      ...
      if (elem.id) {
        this.#id = elem.id; // use existing element id
      } else {
        this.#id = elem.id = "ytplayer-" +
      ...
      this.#loadIframeAPI((err, api) => {
        ...
        if (this.videoId) this.load(this.videoId, this.#autoplay, this.#start);
      });
    }
    ...
    play() {
      ...
      else this.#queueCommand("play");
    }

    pause() {
      if (this.#ready) this.#player.pauseVideo();
      else this.#queueCommand("pause");
    }

    stop() {
      if (this.#ready) this.#player.stopVideo();
      else this.#queueCommand("stop");
    }

    seek(seconds) {
      if (this.#ready) this.#player.seekTo(seconds, true);
      else this.#queueCommand("seek", seconds);
    }

    setVolume(volume) {
      if (this.#ready) this.#player.setVolume(volume);
      else this.#queueCommand("setVolume", volume);
    }
    ...
    mute() {
      ...
      else this.#queueCommand("mute");
    }

    unMute() {
      if (this.#ready) this.#player.unMute();
      else this.#queueCommand("unMute");
    }
    ...
    setSize(width, height) {
      if (this.#ready) this.#player.setSize(width, height);
      else this.#queueCommand("setSize", width, height);
    }

    setPlaybackRate(rate) {
      if (this.#ready) this.#player.setPlaybackRate(rate);
      else this.#queueCommand("setPlaybackRate", rate);
    }

    setPlaybackQuality(suggestedQuality) {
      if (this.#ready) this.#player.setPlaybackQuality(suggestedQuality);
      else this.#queueCommand("setPlaybackQuality", suggestedQuality);
    }

    getPlaybackRate() {
      return (this.#ready && this.#player.getPlaybackRate()) || 1;
    }

    getAvailablePlaybackRates() {
      return (this.#ready && this.#player.getAvailablePlaybackRates()) || [1];
    }
    ...
    getProgress() {
      return (this.#ready && this.#player.getVideoLoadedFraction()) || 0;
    }

    getState() {
      return (this.#ready && YOUTUBE_STATES[this.#player.getPlayerState()]) ||
        "unstarted";
    }
    ...
    #loadIframeAPI(cb) {
      ...
      const isLoading = scripts.some((script) =>
    ...
    #onReady(videoId) {
      ...
      this.load(this.videoId, this.#autoplay, this.#start);
